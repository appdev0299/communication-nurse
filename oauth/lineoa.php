...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    try {
        $stmt = $conn->prepare("SELECT status_Sendline_user, email, title, status_user FROM ccfn_form_s WHERE id = :id");
        ...
        if ($result['status_Sendline_user'] == 0) {
            ...
            $status_user_text = '';
            switch ($status_user) { // แก้ไขจาก $row เป็น $status_user
                case 1:
            ...
            $message = $status_user_text . ' : ' . ' ในหัวข้อ ' . $title;
            ...
            curl_setopt_array($curl, array(
                ...
                CURLOPT_POSTFIELDS => json_encode(array(
                    'program' => 'แจ้งเตือนการใช้บริการ CCFN',
                    'email' => $email,
                    'message' => $message,
