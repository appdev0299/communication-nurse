...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    try {
        $stmt = $conn->prepare("SELECT status_sendline_admin, email, title, status_user FROM ccfn_form_s WHERE id = :id");
        ...
        if ($result['status_user'] == 3) {
            ...
            if ($result['status_sendline_admin'] == 0) {
                ...
                $stmt = $conn->prepare("UPDATE ccfn_form_s SET status_sendline_admin = 1 WHERE id = :id");
                $stmt->bindParam(':id', $id, PDO::PARAM_INT);
                $stmt->execute();
            } else {
