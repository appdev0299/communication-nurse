...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    try {
        $stmt = $conn->prepare("SELECT status_sendline_admin, communicate, email, responsible_1, ref, status_user, status_sendline_user, status_user2, status_user3, status_user4, date_a, status_admin FROM ccfn_form_p WHERE id = :id");
        ...
        switch ($status_user) {
            ...
            case 2:
                if ($status_user2 == 0) {
                    ...
                    // อัปเดตสถานะการส่งไลน์ในฐานข้อมูล
                    $stmt = $conn->prepare("UPDATE ccfn_form_p SET status_user2 = 1 WHERE id = :id");
                    $stmt->bindParam(':id', $id, PDO::PARAM_INT);
                    $stmt->execute();
                }
                break;
            ...
            case 3:
                if ($status_user3 == 0) {
                    ...
                    // อัปเดตสถานะการส่งไลน์ในฐานข้อมูล
                    $stmt = $conn->prepare("UPDATE ccfn_form_p SET status_user3 = 1 WHERE id = :id");
                    $stmt->bindParam(':id', $id, PDO::PARAM_INT);
                    $stmt->execute();
                }
                break;
            ...
            case 4:
                if ($status_user4 == 0) {
                    ...
                    // อัปเดตสถานะการส่งไลน์ในฐานข้อมูล
                    $stmt = $conn->prepare("UPDATE ccfn_form_p SET status_user4 = 1 WHERE id = :id");
                    $stmt->bindParam(':id', $id, PDO::PARAM_INT);
                    $stmt->execute();
                }
                break;
